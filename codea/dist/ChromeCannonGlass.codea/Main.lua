...
function volumeKnobLayout()
    if uiPage == "mixer" then
        local rects = mixerStripRects()
        local ri, ro = rects.input, rects.output
        return {
            input = { x = ri.x + ri.w * 0.5, y = ri.y + ri.h - 46, r = 26 },
            output = { x = ro.x + ro.w * 0.5, y = ro.y + ro.h - 46, r = 26 },
        }
    end
    return {
        input = { x = WIDTH * 0.48, y = 92, r = 46 },
        output = { x = WIDTH * 0.48 + 112, y = 92, r = 46 },
    }
end
