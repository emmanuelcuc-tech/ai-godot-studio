...
local hEarly = Mixer.neonHue(1)
local hMid = Mixer.neonHue(6)
...
check("neon blends slowly at the start", hEarly > h0 and hEarly < hMid)
check("neon blends across the full 12s", hMid > hEarly and hMid < hNext)
check("neon step is 12 seconds", Mixer.NEON_STEP == 12)
local r0, g0, b0 = Mixer.neonRGB(0)
local rm, gm, bm = Mixer.neonRGB(6)
check("neon rgb slow blend", math.abs(rm - r0) > 1 or math.abs(gm - g0) > 1 or math.abs(bm - b0) > 1)
