...
local hHold = Mixer.neonHue(8)
local hNext = Mixer.neonHue(12)
local hBlend = Mixer.neonHue(10.75)
check("neon starts blue", math.abs(h0 - 205) < 0.01)
check("neon holds color for 12 seconds", math.abs(hHold - h0) < 0.01)
check("neon next stop after 12s", math.abs(hNext - 282) < 0.01)
check("neon blends in before the 12s handoff", hBlend > h0 + 1 and hBlend < hNext - 1)
check("neon step is 12 seconds", Mixer.NEON_STEP == 12)
local r0, g0, b0 = Mixer.neonRGB(0)
local rb, gb, bb = Mixer.neonRGB(10.75)
check("neon rgb blends", math.abs(rb - r0) > 1 or math.abs(gb - g0) > 1 or math.abs(bb - b0) > 1)
