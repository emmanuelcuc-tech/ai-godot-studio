	...
	for id in ["fps", "voxel"]:
		...
		if not proj_path.is_empty():
			...
			if id == "fps":
				...
				var EditScr = load("res://scripts/editors/edit_game_ui.gd")
				print("HAS_EDIT_GAME_UI=", EditScr != null)
				var LocalLib = load("res://scripts/editors/local_asset_library.gd")
				print("LOCAL_OK_ASSET=", LocalLib.is_allowed_root("F:/asset"))
				print("LOCAL_BLOCK_F=", not LocalLib.is_allowed_root("F:/"))
				print("LOCAL_BLOCK_BF=", not LocalLib.is_allowed_root("F:/Battlefield"))
				print("LOCAL_BLOCK_ROM=", not LocalLib.is_allowed_root("F:/roms"))
				var scanned: Array = LocalLib.scan(true)
				print("LOCAL_SCAN=", scanned.size())
				var seeded: Dictionary = LocalLib.apply_on_create(proj_path, "fps", {"textures": true, "sprites": true, "models": true})
				print("LOCAL_SEED_OK=", seeded.get("ok", false))
				print("LOCAL_SEED_COPIED=", int(seeded.get("copied", 0)))
				print("HAS_F_ATTR=", FileAccess.file_exists(proj_path.path_join("docs/F_ASSET_ATTRIBUTION.md")))
				print("HAS_F_PHYS=", DirAccess.dir_exists_absolute(proj_path.path_join("addons/f_asset_physics")))
				print("HAS_F_PHYS_CFG=", FileAccess.file_exists(proj_path.path_join("addons/f_asset_physics/plugin.cfg")))
				print("HAS_LOCAL_LIB=", LocalLib != null)
				print("LOCAL_DEFAULT=", LocalLib.DEFAULT_ROOT)
				print("LOCAL_CREATE_OK=", seeded.get("ok", false), " COPIED=", int(seeded.get("copied", 0)))
				var main_tscn: String = FileAccess.get_file_as_string("res://scenes/main.tscn")
				print("HAS_EDIT_TAB=", main_tscn.contains("EditGame") and main_tscn.contains("edit_game_ui.gd"))
				print("HAS_LOCAL_SETTING=", main_tscn.contains("LocalAssetFolder"))
